...
def addTransition(initialState, action):
	...
	if(sumDots==0 or initPacman==initGhost): #End state
		return
	nextGhost=moveDirection(initGhost, ghostPolicy[initialState])
	nextPacman=moveDirection(initPacman, action)
	if(nextPacman==initPacman):
		return
...
def parseGhost():
	global ghostPolicy
	with open(args.ghostPolicy, 'r') as f:
